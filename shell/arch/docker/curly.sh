...
function run_curl() {
	...
	while true; do
		...
		response_code=$(curl --continue-at - --connect-timeout 5 --max-time 10 --retry "${retry_count}" --retry-delay "${retry_wait}" --retry-max-time "${retry_max_time}" -o "${output_file}" -L "${silent_mode}" -w "%{http_code}" "${url}")
		exit_code=$?
		...
		if [ "${response_code}" -ge "200" ] && [ "${response_code}" -le "299" ]; then
			...
			if [ "${retry_count}" -eq "0" ]; then
				...
				echo -e "[warn] Response code ${response_code} from curl != 2xx, exausted retries exiting script..."; exit 1
