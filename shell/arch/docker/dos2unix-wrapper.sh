...
# see here for details https://access.redhat.com/solutions/3168671
...
if [ "${crlf_line_ending_count}" != 0 ]; then
	...
	echo "[info] Setting permissions on converted file '${1}'..." | ts '%Y-%m-%d %H:%M:%.S'
	chown -R "${PUID}":"${PGID}" "${1}"
	chmod -R 775 "${1}"
	echo "[info] Permissions set" | ts '%Y-%m-%d %H:%M:%.S'
fi