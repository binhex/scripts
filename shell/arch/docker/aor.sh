...
if [[ ! -z "${aor_packages}" ]]; then
	...
	for aor_package_name in "${aor_package_list[@]}"; do
		...
		curly.sh -rc 6 -rw 10 -of "/tmp/aor_json_${aor_package_name}" -url "https://www.archlinux.org/packages/search/json/?q=${aor_package_name}&repo=Community&repo=Core&repo=Extra&repo=Multilib&arch=any&arch=x86_64"
